...
class ServerConnection : public AbstractServerConnection {
    ...
    public:
        ServerConnection(const char* path); // default constructor
        ...
        void requestUserListFromQueue(std::string qName);
        ...
        virtual uint16_t getDefaultValidity() override;

        virtual uint16_t getMaxLength() override;
};
