...
/* =============== DEFAULT CONTRUCTOR ================== */
ServerConnection::ServerConnection(const char* path) {
    int fd = open(path, O_RDONLY); // open config file and handle errors
    if(fd == -1) {
...
void ServerConnection::requestUserListFromQueue(std::string queue) {
    responseMutex.lock();
    ...
    write(serverSocket, &action, sizeof(uint16_t));
    write(serverSocket, &size, sizeof(uint16_t));
    write(serverSocket, queue.c_str(), size);
    writeMutex.unlock();
}
...
uint16_t ServerConnection::getDefaultValidity() {
    return defaultTime;
}

uint16_t ServerConnection::getMaxLength() {
    return maxLength;
}